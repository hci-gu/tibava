#!/usr/bin/env python3
"""Filter and normalize generated ELAN EAF documents."""
...
def _parse_tree(raw: bytes) -> ET.ElementTree:
    lowered = raw.lower()
...
def filter_eaf_xml(
    ...
    *,
    ...
    """Filter an EAF document entirely in memory for HTTP export responses."""

    raw = content.encode("utf-8") if isinstance(content, str) else content
    tree = _parse_tree(raw)
    result = filter_tree(tree, threshold)
    return _serialize_tree(tree).decode("utf-8"), result
